    ...
    public TTVBuilder addSensorData(SensorInfo info) {
        ...
        for (int loop1 = 0 ; loop1 < type.getValueNumbers() ; loop1++) {
            ...
            switch (type.getValueTDVTypes()[loop1]) {
                ...
                case 0x05:      formatter.format("%02x", ValueType.USHORT.getCode());   formatter.format("%04x", (int) values[loop1]);                  break;          // battery level
                ...
                case 0x20:      formatter.format("%02x", ValueType.LATITUDE.getCode());    formatter.format("%08x", (int)(values[loop1]* 100000));  break;          // latitude
                case 0x21:      formatter.format("%02x", ValueType.LONGITUDE.getCode());    formatter.format("%08x", (int)(values[loop1]* 100000));  break;          // longitude
                ...
                case 0x87:      formatter.format("%02x", ValueType.SHORT.getCode());   formatter.format("%04x", (int) values[loop1]);                  break;          // buzzer
                case 0x88:      formatter.format("%02x", ValueType.SHORT.getCode());    formatter.format("%04x", (int) values[loop1]);                  break;          // led
                ...
                case 0x33:      formatter.format("%02x", ValueType.INT.getCode());   formatter.format("%08x", (int) values[loop1]);                  break;          // step count
                case 0x84:      formatter.format("%02x", ValueType.BOOL.getCode());     formatter.format("%02x", 0);                                    break;          // camera
...
//        formatter.format("%016x", nowMillis); // write value
    ...
    public static List<SensorInfo> parseSensorData(String tdvString) {
        ...
        int size = tdvString.length();

        for (int loop1 = 0 ; loop1 < size ; ) {
            // read type
            ...
            switch (type) {
                ...
                case 0x87:  sensorType = SensorType.BUZZER;                 intValues[0] = Long.parseLong(value, 16);                           break;  // buzzer
                case 0x88:  sensorType = SensorType.LED;                    intValues[0] = Long.parseLong(value, 16);                           break;  // led
                ...
                case 0x84:  sensorType = SensorType.CAMERA;                 intValues[0] = Long.parseLong(value, 16);                           break;  // camera
